...
class YieldService:
    ...
    def predict(cls, data):
        ...
        year = data.get('Year')
        rain = data.get('average_rain_fall_mm_per_year')
        pesticides = data.get('pesticides_tonnes')
        temp = data.get('avg_temp')
        crop = data.get('Item', '').lower()

        # One-Hot Encoding setup (Hardcoded based on training data structure)
        crop_dict = {
            'Item_Maize': 0,
        ...
        # Scaling is skipped here as Random Forest doesn't require it, 
        # but the scaler is loaded if needed for other models.
